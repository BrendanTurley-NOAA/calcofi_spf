...
mtext('10 years with greatest biomass')
...
mtext(expression(paste('Longitude (',degree,'W)')),1,2.5)
mtext(expression(paste('Latitude (',degree,'N)')),2,2)
mtext('10 years with least biomass')
...
setwd("~/Documents/R/Github/calcofi_spf/figures")
png('fig2_spatial_comparison_1.png',height=7.5,width=10,units='in',res=300)
par(mfrow=c(2,2),mar=c(4,4,1.5,1))
plot(1,1,xlim=c(-125,-117),ylim=c(30,35),xlab='',ylab='',las=1,asp=1)
image(anch_hi_interp,
      breaks=c(3.5,10),col=cols[length(cols)],
...
plot(1,1,xlim=c(-125,-117),ylim=c(30,35),xlab='',ylab='',las=1,asp=1)
image(sard_hi_interp,
      breaks=c(1,10),col=cols2[length(cols2)],
      las=1,asp=1,add=T)
image(sard_hi_interp,
      breaks=brks2,col=cols2,
      las=1,asp=1,add=T)
points(calcofi_stations$Longitude,calcofi_stations$Latitude,
       col='turquoise1',cex=.5,lwd=1.2,pch=20)
plot(world,col='gray70',add=T)
mtext(expression(paste('Longitude (',degree,'W)')),1,2.5)
mtext(expression(paste('Latitude (',degree,'N)')),2,2)
...
lth <- length(brks)-1
png("fig2_colorbar_anch_1.png", height = 5, width = 1.5, units = 'in', res=300)
par(mar=c(1,1,1,5))
image(1:2,1:length(brks),t(matrix(c(seq(1,lth),seq(1,lth)),lth,2)),col=cols,xaxt='n',yaxt='n',xlab='',ylab='')
...
lth2 <- length(brks2)-1
png("fig2_colorbar_sard_1.png", height = 5, width = 1.5, units = 'in', res=300)
par(mar=c(1,1,1,5))
